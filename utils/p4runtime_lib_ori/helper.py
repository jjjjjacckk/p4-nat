...
class P4InfoHelper(object):
    ...
    # credit: https://github.com/kevinbird61/p4-researching/blob/master/utils/p4runtime_lib/helper.py
    def buildDigestEntry(self, digest_name=None):
        digest_entry = p4runtime_pb2.DigestEntry()

        print '[ buildDigestEntry ] ', digest_entry
        # using name 
        digest_entry.digest_id = self.get_digests_id(digest_name)
        # using id directly
        #digest_entry.digest_id = int(digest_id)
        # FIXME: set config
        digest_entry.config.max_timeout_ns = 1000
        digest_entry.config.max_list_size = 1024
        digest_entry.config.ack_timeout_ns = 1000
        print '[ buildDigestEntry ] ', digest_entry
        return digest_entry