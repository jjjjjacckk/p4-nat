...
class SwitchConnection(object):
    ...
    def MasterArbitrationUpdate(self, dry_run=False, **kwargs):
        ...
        print '[ MasterArbitrationUpdate ]', request, type(request)
        request.arbitration.device_id = self.device_id
        request.arbitration.election_id.high = 0
        request.arbitration.election_id.low = 1
        print '[ MasterArbitrationUpdate ]', request.arbitration, request
        self.requests_stream.put(request)

        # for ele in self.requests_stream:
        #     print '[ in loop ]', ele
    ...
    # Digest
    # author: https://github.com/kevinbird61/p4-researching/blob/master/utils/p4runtime_lib/switch.py
    ...
    def DigestList(self, dry_run=False, **kwargs):
        print '[ DigestList ] '
        request = p4runtime_pb2.StreamMessageRequest()
        print '[ DigestList ]',  request, dry_run

        if dry_run:
            print "P4 Runtime DigestList Response: ", request 
        else: 
            print '[ DigestList ] inside!'
            self.requests_stream.put(request)
            print '[ DigestList ]',  repr(self.stream_msg_resp), type(self.stream_msg_resp)
            # print '[ DigestList ]', '|', list(self.stream_msg_resp), '|'
            for item in self.stream_msg_resp:
                # print '[ DigestList ]', 'in the loop' 
                return item 
