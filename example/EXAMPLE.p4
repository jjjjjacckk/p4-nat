...
/*
    Basic Parser
*/
parser Basic_parser( packet_in packet,
                     out headers hdr,
...
/*
    Verify checksum
*/
control Basic_verifyCk( inout headers hdr,
                        inout metadata_t metadata ) {
    // TODO
    apply {
    }
...
/*
    Basic ingress
*/
control Basic_ingress( inout headers hdr,
                       inout metadata_t metadata,
                       inout standard_metadata_t standard_metadata ) {
    ...
    action new_synack() {
        digest<syn_ack_digest>( (bit<32>) 1024, { hdr.ipv4.dstAddr } );
    }
    action checking() {
        digest<check_digest>( (bit<32>) 1024, { hdr.ipv4.dstAddr,
                                                apply_index } );
    }
    action merge(){
        digest<merge_digest>((bit<32>) 1024, { apply_index } );
    }
...
/*
    Basic Egress
*/
control Basic_egress( inout headers hdr,
...
*/
control Basic_computeCk( inout headers hdr,
                         inout metadata_t metadata ) {
    apply {
        update_checksum(
            hdr.ipv4.isValid(),
...
/*
    Basic Deparser
*/
control Basic_deparser( packet_out packet,
                        in headers hdr ) {
    apply {
