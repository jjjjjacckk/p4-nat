...
def printRegister(p4info_helper, sw, register_name, index):
    ...
    :param p4info_helper: the P4Info helper
    :param sw:  the switch connection
    :param register_name: the name of the counter from the P4 program
    :param index: the counter index (in our case, the tunnel ID)
    """
    for response in sw.ReadRegister(p4info_helper.get_counters_id(register_name), index):
        for entity in response.entities:
            counter = entity.counter_entry
            print "%s %s %d: %d packets (%d bytes)" % (
                sw.name, register_name, index,
                counter.data.packet_count, counter.data.byte_count
            )
...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        writeTunnelRules(p4info_helper, ingress_sw=s3, egress_sw=s2, tunnel_id=400,
                         dst_eth_addr="08:00:00:00:02:22", dst_ip_addr="10.0.2.2")
